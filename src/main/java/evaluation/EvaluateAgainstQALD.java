...
import qald.Answer;
import qald.QaldData;
import qald.QalddEntry;
import qald.Question;
import qald.UriAnswer;
...
public class EvaluateAgainstQALD implements Constants {
    ...
    public CSVWriter csvWriterFile;
    ...
    public void evaluateAndOutput(Map<String, String[]> queggQuestions, String qaldOriginalFile, String qaldModifiedFile, String qaldRaw, String languageCode, Double similarityMeasure, String lexialEntry) throws IOException, Exception {
        ...
        if (menu.contains(ANSWER_SELECTED)) {
            ...
            getGoldAnswer(qaldOriginalFile, languageCode, online, ids, this.QALD_ANSWER_FILE);
            System.out.println("FIND_QALD_ANSWER completed!!");
        } else if (menu.contains(FIND_QALD_ANSWER)) {
            getGoldAnswer(qaldOriginalFile, languageCode, online, new TreeSet<Integer>(), this.QALD_ANSWER_FILE);
            System.out.println("FIND_QALD_ANSWER completed!!");
        }
    ...
    /*private void getGoldAnswer(QALD qaldFile, String languageCode, Boolean flag, Set<Integer> ids, String resultFile) throws Exception {
        Integer total = qaldFile.questions.size(),index=0;

        for (QALD.QALDQuestions qaldQuestions : qaldFile.questions) {
            List<String> qualResults = new ArrayList<String>();
            String qaldQuestion = QALDImporter.getQaldQuestionString(qaldQuestions, languageCode);
            String qaldSparqlQuery = QALDImporter.getQaldSparqlQuery(qaldQuestions);
            qualResults = this.getSparqlQuery(qaldSparqlQuery, flag, qualResults);
            System.out.println(total + " " +index +" "+qaldQuestions.id + " " + qaldQuestion+" "+qualResults.size());
            this.csvWriterFile = new CSVWriter(new FileWriter(resultFile, true));
            this.csvWriterFile.writeNext(new String[]{qaldQuestions.id, qaldQuestion, qaldSparqlQuery, qualResults.toString()});
            this.csvWriterFile.close();
            index=index+1;
            
        }
      
    }*/
    private void getGoldAnswer(String qaldFile, String languageCode, Boolean flag, Set<Integer> ids, String resultFile) throws Exception {
        QaldData qaldData = getInputQaldData(new File(qaldFile));
        this.csvWriterFile = new CSVWriter(new FileWriter(resultFile, true));

        for (QalddEntry qalddEntry : qaldData.getQuestions()) {
            String id = qalddEntry.getId().toString();
            String qaldQuestion = QalddEntry.getQuestion(qalddEntry, languageCode);
            String qaldSparqlQuery = qalddEntry.getQuery().get("sparql");
            List<String> qualResults = new ArrayList<String>();
            if (qaldSparqlQuery.contains("ASK WHERE")) {
                qualResults.add("true");
            } else {
                System.out.println(qaldQuestion);
                qualResults = QalddEntry.getAnswers(qalddEntry, languageCode);
            }

            this.csvWriterFile.writeNext(new String[]{id, qaldQuestion, qaldSparqlQuery, qualResults.toString()});
    ...
    public QaldData getInputQaldData(File file) throws Exception {
        ObjectMapper mapper = new ObjectMapper();
        return mapper.readValue(file, QaldData.class);
    }
